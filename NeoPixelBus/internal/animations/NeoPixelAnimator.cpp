...
NeoPixelAnimator::NeoPixelAnimator(uint16_t countAnimations, uint16_t timeScale) : _countAnimations(countAnimations),
                                      ...
                                      AnimUpdateCallback animUpdate)
{
    ...
    {
        _animationLastTick = Get_Time_ms();
    }
...
void NeoPixelAnimator::UpdateAnimations()
{
    if (_isRunning)
    {
        uint32_t currentTick = Get_Time_ms();
