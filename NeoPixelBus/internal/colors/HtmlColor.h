...
struct HtmlColor
{
        ...
        if (nameSize > 0)
        {
            ...
            else
            {
                ...
                for (uint8_t indexName = 0; indexName < T_HTMLCOLORNAMES::Count(); ++indexName)
                {
                    ...
                    const char *searchName = reinterpret_cast<const char *>(pgm_read_ptr(&(colorPair->Name)));
    ...
    template <typename T_HTMLCOLORNAMES>
    size_t Parse(std::string const &name)
    {
    ...
    size_t ToString(char *buf, size_t bufSize) const
    {
        ...
        for (uint8_t indexName = 0; indexName < T_HTMLCOLORNAMES::Count(); ++indexName)
        {
            ...
            {
                const char *name = (const char *)pgm_read_ptr(&colorPair->Name);
